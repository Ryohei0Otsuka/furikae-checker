...
const STORAGE_PREFIX = 'furikae-checker:month:'
...
function loadMonth(
  ...
  try {
    ...
    const savedMap = new Map(
      parsed.map((item) => [item.date, item]),
    )

    const validDates = new Set(
      defaults.map((item) => item.date),
    )
...
function App() {
  ...
  const handlePairChange = (
    ...
    setDays((currentDays) => {
      ...
      const target = currentDays.find(
        (day) => day.date === pairedDate,
      )
  ...
  const getPairCandidates = (
    source: WorkDay,
  ): WorkDay[] => {
    if (
      source.workType === 'substituteWork'
    ) {
      return days.filter(
        (day) =>
          day.workType ===
  ...
  const handleCopyRemark = async (
    ...
    try {
      ...
      setCopyMessage(
        `${formatDate(day.date)}の備考をコピーしました。`,
      )
    } catch {
      setCopyMessage(
        'コピーできませんでした。' +
          '備考文を選択して手動でコピーしてください。',
      )
    }
  }
